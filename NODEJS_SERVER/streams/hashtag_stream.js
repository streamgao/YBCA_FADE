...
class HashtagStream{
    constructor(date, hashtag, stream, Network){
        ...
        console.log("created play for ", date, hashtag, stream.length, "events")
    }
    add(id, ip){
        if(this.ip == null){
            console.log("adding", ip, "to", id, this.date, this.hashtag)
    ...
    checkHealth(){ // check every 3 second
        if(this.ip!==null){
            ...
            if(this.heartbeat_cnt<0){
                console.log("removing", this.ip, "from", this.date, this.hashtag)
                this.ip = null
            }else{
                setTimeout(()=>{ this.checkHealth()}, 3000);
            }
    ...
    renderTweet(){
        ...
        if(this.stream[this.playHead]<this.t_now){
            if(this.ip!==null){
                ...
                if(Config.DEBUG) console.log(this.date, this.hashtag, this.t_now)
            }
            this.playHead++
        }
